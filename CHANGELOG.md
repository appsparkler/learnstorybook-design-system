# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- chore: updated publish-config (subs@appsparkler.com)
- chore: updated (subs@appsparkler.com)
- Added only dist to list of files (subs@appsparkler.com)
- chore: upgraded run command (subs@appsparkler.com)
- chore: upgraded dependencies (subs@appsparkler.com)
- chore: added release script (subs@appsparkler.com)
- chore: skipping pre-flight check (subs@appsparkler.com)
- chore: re-installed setup (subs@appsparkler.com)
- chore: updated env variable (subs@appsparkler.com)
- chore: updated workflow to run in production environment (subs@appsparkler.com)
- 0.1.4 (subs@appsparkler.com)
- 0.1.3 (subs@appsparkler.com)
- v0.1.2 (subs@appsparkler.com)

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- chore: updated (subs@appsparkler.com)
- Added only dist to list of files (subs@appsparkler.com)
- chore: upgraded run command (subs@appsparkler.com)
- chore: upgraded dependencies (subs@appsparkler.com)
- chore: added release script (subs@appsparkler.com)
- chore: skipping pre-flight check (subs@appsparkler.com)
- chore: re-installed setup (subs@appsparkler.com)
- chore: updated env variable (subs@appsparkler.com)
- chore: updated workflow to run in production environment (subs@appsparkler.com)
- 0.1.4 (subs@appsparkler.com)
- 0.1.3 (subs@appsparkler.com)
- v0.1.2 (subs@appsparkler.com)

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- Added only dist to list of files (subs@appsparkler.com)
- chore: upgraded run command (subs@appsparkler.com)
- chore: upgraded dependencies (subs@appsparkler.com)
- chore: added release script (subs@appsparkler.com)
- chore: skipping pre-flight check (subs@appsparkler.com)
- chore: re-installed setup (subs@appsparkler.com)
- chore: updated env variable (subs@appsparkler.com)
- chore: updated workflow to run in production environment (subs@appsparkler.com)
- 0.1.4 (subs@appsparkler.com)
- 0.1.3 (subs@appsparkler.com)
- v0.1.2 (subs@appsparkler.com)

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- Added only dist to list of files (subs@appsparkler.com)
- chore: upgraded run command (subs@appsparkler.com)
- chore: upgraded dependencies (subs@appsparkler.com)
- chore: added release script (subs@appsparkler.com)
- chore: skipping pre-flight check (subs@appsparkler.com)
- chore: re-installed setup (subs@appsparkler.com)
- chore: updated env variable (subs@appsparkler.com)
- chore: updated workflow to run in production environment (subs@appsparkler.com)
- 0.1.4 (subs@appsparkler.com)
- 0.1.3 (subs@appsparkler.com)
- v0.1.2 (subs@appsparkler.com)

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- Added only dist to list of files (subs@appsparkler.com)
- chore: upgraded run command (subs@appsparkler.com)
- chore: upgraded dependencies (subs@appsparkler.com)
- chore: added release script (subs@appsparkler.com)
- chore: skipping pre-flight check (subs@appsparkler.com)
- chore: re-installed setup (subs@appsparkler.com)
- chore: updated env variable (subs@appsparkler.com)
- chore: updated workflow to run in production environment (subs@appsparkler.com)
- 0.1.4 (subs@appsparkler.com)
- 0.1.3 (subs@appsparkler.com)
- v0.1.2 (subs@appsparkler.com)

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- Added only dist to list of files (subs@appsparkler.com)
- chore: upgraded run command (subs@appsparkler.com)
- chore: upgraded dependencies (subs@appsparkler.com)
- chore: added release script (subs@appsparkler.com)
- chore: skipping pre-flight check (subs@appsparkler.com)
- chore: re-installed setup (subs@appsparkler.com)
- chore: updated env variable (subs@appsparkler.com)
- chore: updated workflow to run in production environment (subs@appsparkler.com)
- 0.1.4 (subs@appsparkler.com)
- 0.1.3 (subs@appsparkler.com)
- v0.1.2 (subs@appsparkler.com)

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- chore: upgraded run command ([@appsparkler](https://github.com/appsparkler))
- chore: upgraded dependencies ([@appsparkler](https://github.com/appsparkler))
- chore: added release script ([@appsparkler](https://github.com/appsparkler))
- chore: skipping pre-flight check ([@appsparkler](https://github.com/appsparkler))
- chore: re-installed setup ([@appsparkler](https://github.com/appsparkler))
- chore: updated env variable ([@appsparkler](https://github.com/appsparkler))
- chore: updated workflow to run in production environment ([@appsparkler](https://github.com/appsparkler))
- 0.1.4 ([@appsparkler](https://github.com/appsparkler))
- 0.1.3 ([@appsparkler](https://github.com/appsparkler))
- v0.1.2 ([@appsparkler](https://github.com/appsparkler))

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- chore: upgraded run command ([@appsparkler](https://github.com/appsparkler))
- chore: upgraded dependencies ([@appsparkler](https://github.com/appsparkler))
- chore: added release script ([@appsparkler](https://github.com/appsparkler))
- chore: skipping pre-flight check ([@appsparkler](https://github.com/appsparkler))
- chore: re-installed setup ([@appsparkler](https://github.com/appsparkler))
- chore: updated env variable ([@appsparkler](https://github.com/appsparkler))
- chore: updated workflow to run in production environment ([@appsparkler](https://github.com/appsparkler))
- 0.1.4 ([@appsparkler](https://github.com/appsparkler))
- 0.1.3 ([@appsparkler](https://github.com/appsparkler))
- v0.1.2 ([@appsparkler](https://github.com/appsparkler))

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Fri Apr 02 2021)

#### ⚠️ Pushed to `main`

- chore: skipping pre-flight check ([@appsparkler](https://github.com/appsparkler))
- chore: re-installed setup ([@appsparkler](https://github.com/appsparkler))
- chore: updated env variable ([@appsparkler](https://github.com/appsparkler))
- chore: updated workflow to run in production environment ([@appsparkler](https://github.com/appsparkler))
- 0.1.4 ([@appsparkler](https://github.com/appsparkler))
- 0.1.3 ([@appsparkler](https://github.com/appsparkler))
- v0.1.2 ([@appsparkler](https://github.com/appsparkler))

#### Authors: 1

- Akash ([@appsparkler](https://github.com/appsparkler))

---

# v0.1.1 (Tue Mar 09 2021)

- Created first version of the design system, with `Avatar`, `Badge`, `Button`, `Icon` and `Link` components.

#### Authors: 1

- [appsparkler](https://github.com/appsparkler)
